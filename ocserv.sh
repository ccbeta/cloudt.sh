...
Download_ocserv(){
	...
	mkdir "${conf_file}"
	wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/ccbeta/shell/master/ocserv.conf"
	[[ ! -s "${conf}" ]] && echo -e "${Error} ocserv 配置文件下载失败 !" && rm -rf "${conf_file}" && exit 1
}
Service_ocserv(){
	if ! wget --no-check-certificate https://raw.githubusercontent.com/ccbeta/shell/master/ocserv_debian -O /etc/init.d/ocserv; then
...
Installation_dependency(){
	...
	if [[ ${release} = "centos" ]]; then
		...
		wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/ccbeta/shell/master/us.sources.list"
		apt-get update
...
Set_username(){
	...
	stty erase '^H' && read -p "(默认: create):" username
	[[ -z "${username}" ]] && username="create"
...
Set_tcp_port(){
	...
	stty erase '^H' && read -p "(默认: 5443):" set_tcp_port
	[[ -z "$set_tcp_port" ]] && set_tcp_port="5443"
