...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/ccbeta/shell/master/bbr.sh; then
...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: abc):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="abc"
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 8989):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="8989"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	stty erase '^H' && read -p "(默认: 123589):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="123589"
...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/ccbeta/shadowsocksr/archive/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/ccbeta/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ccbeta/shell/master/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ccbeta/shell/master/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
Install_ServerSpeeder(){
	...
	#借用91yun.rog的开心版锐速
	wget --no-check-certificate -qO /tmp/serverspeeder.sh https://raw.githubusercontent.com/91yun/serverspeeder/master/serverspeeder-all.sh
